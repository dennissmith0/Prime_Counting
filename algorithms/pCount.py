...
# Let's start optimizing the algorithm.
def create_and_count_optimized_with_set(n):
    # Initialize an empty set for multiples
    multiples = set()
    ...
    # Create set of all products of possible primes (multiples)
    for i_index, i in enumerate(poss):
        for j in poss[i_index:]:
            product = i * j
            if product > n:
    ...
    # Number of primes is difference of possible primes and multiples, plus 2 (for 2 and 3)
    return len(poss) - len(multiples) + 2
